...
const FacilityEditModal: React.FC<FacilityEditModalProps> = ({ isOpen, onClose, initialData, onSave }) => {
  ...
  const formatDateForInput = (dateStr?: string) => {
    if (!dateStr || dateStr === 'Hoje' || dateStr === 'A definir') return '';

    // Try to handle ISO format directly
    if (dateStr.includes('T')) return dateStr.split('T')[0];

    // Handle "DD/MM/YYYY" or "DD/MM/YYYY (Text)"
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        lastMaintenance: formatDateForInput(initialData.lastMaintenance),
        nextMaintenance: formatDateForInput(initialData.nextMaintenance)
      });
    } else {
      setFormData({
        name: '',
  ...
  return createPortal(
    <div className="fixed inset-0 z-[150] flex items-start md:items-center justify-center p-4 overflow-y-auto bg-slate-900/60 backdrop-blur-sm custom-scrollbar">
      ...
      <div className="relative bg-white dark:bg-surface-dark w-full max-w-6xl max-h-[90vh] rounded-[32px] md:rounded-[40px] shadow-2xl overflow-hidden animate-fadeIn flex flex-col md:flex-row border border-white/20 my-auto">

        {/* Left Column */}
        <div className="flex-1 p-5 md:p-10 space-y-6 md:space-y-8 border-r border-slate-100 dark:border-slate-800 flex flex-col overflow-y-auto custom-scrollbar">
          ...
          <div className="space-y-6 flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Última Manutenção</label>
                <input
                  type="date"
                  value={formData.lastMaintenance} // Ensure these fields exist in state
                  onChange={(e) => setFormData({ ...formData, lastMaintenance: e.target.value })}
                  className="w-full px-6 py-4 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl outline-none text-sm font-medium dark:text-white"
                />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Próxima Manutenção</label>
                <input
                  type="date"
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 rounded-[32px] p-6 md:p-8 border border-slate-100 dark:border-slate-800 space-y-6">
              <div className="flex justify-between items-end">
                ...
                <div className="text-right">
                  ...
                  <p className="text-2xl font-black text-slate-900 dark:text-white leading-none">R$ {(Number(formData.monthlyFixedCost) || 0).toFixed(2)}</p>
                </div>
              </div>
                ...
                {formData.costBreakdown.map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-slate-700">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-xs font-black">R$ {(Number(item.value) || 0).toFixed(2)}</span>
        ...
        {/* Right Column */}
        <div className="w-full md:w-[380px] bg-slate-50/80 dark:bg-slate-900/40 p-5 md:p-10 flex flex-col gap-6 overflow-y-auto custom-scrollbar border-t md:border-t-0 border-slate-100 dark:border-slate-800">
