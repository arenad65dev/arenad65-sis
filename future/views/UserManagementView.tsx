...
const UserManagementView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-auto lg:h-full lg:overflow-hidden animate-fadeIn relative">
      ...
      <div className="fixed top-20 right-8 z-[200] flex flex-col gap-3 pointer-events-none">
        {toasts.map(toast => (
          <div
            key={toast.id}
            className={`px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-4 animate-fadeIn border backdrop-blur-md pointer-events-auto ${toast.type === 'success'
      ...
      {/* Main Table Section */}
      <div className={`flex-1 flex flex-col gap-6 bg-white dark:bg-surface-dark rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden ${selectedUser ? 'hidden lg:flex' : 'flex'}`}>
              ...
              {filteredUsers.map(user => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest ${user.status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-primary' : 'bg-slate-100 text-slate-400'
                      }`}>
      ...
      <aside className={`w-full lg:w-[420px] flex flex-col bg-white dark:bg-surface-dark rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl h-full overflow-hidden shrink-0 ${selectedUser ? 'flex' : 'hidden lg:flex'}`}>
        {selectedUser ? (
          <>
            <div className="p-6 md:p-8 bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800 relative">
              {/* Botão Voltar Mobile */}
              <button
                onClick={() => setSelectedUser(null)}
                className="lg:hidden absolute top-6 left-6 size-10 flex items-center justify-center bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-500 shadow-sm z-10"
              >
                <span className="material-symbols-outlined">arrow_back</span>
              </button>

              <div className="flex flex-col items-center text-center gap-2 md:gap-4 mt-8 lg:mt-0">
                ...
                <div className="flex gap-2 w-full">
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border ${showLogsQuickView ? 'bg-primary text-slate-900 border-primary' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500'
                      }`}
                  >
            ...
            <div className="p-6 md:p-8 bg-slate-50/50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 flex gap-3">
