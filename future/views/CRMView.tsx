...
const CRMView: React.FC = () => {
  ...
  const handleNotify = (data: any) => {
    if (selectedUser) addToast(`Notificação enviada para ${selectedUser.name}!`);
  };
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-auto lg:h-full lg:overflow-hidden animate-fadeIn relative pb-12">
      ...
      <div className={`flex-1 flex flex-col gap-6 overflow-hidden ${selectedUser ? 'hidden lg:flex' : 'flex'}`}>
        {/* KPIs CRM */}
          ...
          <div className="flex gap-4 w-full md:w-auto">
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`size-12 rounded-2xl border transition-all flex items-center justify-center group ${activeTab === 'Configurações'
        ...
        <div className="p-2 bg-white dark:bg-surface-dark rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex gap-1 overflow-x-auto scrollbar-hide shrink-0">
          {['Todos', 'Nível Ouro', 'Nível Prata', 'Nível Bronze', 'Inativos', 'Aniversariantes'].map((tab) => (
            <button
              ...
              className={`px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all border shrink-0 ${activeTab === tab ? 'bg-primary text-slate-900 border-transparent shadow-lg shadow-primary/10' : 'text-slate-400 border-transparent hover:bg-slate-50 dark:hover:bg-white/5'
                }`}
            >
              {tab}
            </button>
                  ...
                  {filteredUsers.map(user => (
                    ...
                    >
                      ...
                      <td className="px-6 py-6">
                        <span className={`px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest border ${user.level === 'Ouro' ? 'bg-yellow-50 text-yellow-700 border-yellow-100 dark:bg-yellow-900/30 dark:text-yellow-500 dark:border-yellow-900/40' :
      ...
      <aside className={`w-full lg:w-[420px] bg-white dark:bg-surface-dark rounded-[48px] border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden flex flex-col p-6 md:p-10 relative shrink-0 max-h-[calc(100vh-3rem)] lg:max-h-full ${selectedUser ? 'flex' : 'hidden lg:flex'}`}>
        {!selectedUser ? (
          <div className="h-full flex flex-col items-center justify-center p-12 text-center">
            <p className="text-xs font-black uppercase tracking-widest">Selecione um atleta para ver detalhes</p>
          </div>
        ) : (
          <>
            {/* Botão Voltar Mobile */}
            <button
              ...
              className="lg:hidden absolute top-6 left-6 size-10 z-30 flex items-center justify-center bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-500 shadow-sm"
            >
            ...
            {(isUserBirthday || isUserInactive) && (
              <div className={`absolute top-0 left-0 right-0 py-3 px-8 text-center text-[9px] font-black uppercase tracking-widest animate-pulse z-20 ${isUserBirthday ? 'bg-primary text-slate-900' : 'bg-red-500 text-white'}`}>
                {isUserBirthday ? 'Aniversariante do Mês! 🎂' : 'Atleta em Risco de Churn ⚠️'}
              </div>
            )}

            <div className={`flex flex-col items-center mb-6 mt-12 lg:mt-0 ${isUserBirthday || isUserInactive ? 'md:mt-8' : ''}`}>
              <div className="relative mb-4">
                <div className="size-24 md:size-32 rounded-[24px] md:rounded-[40px] overflow-hidden border-4 border-white dark:border-surface-dark shadow-2xl ring-2 ring-primary">
                  <img src={selectedUser?.avatar} className="w-full h-full object-cover" />
                </div>
              </div>
              <h3 className="text-2xl md:text-3xl font-black tracking-tighter text-center dark:text-white leading-none mb-1 md:mb-2">{selectedUser?.name}</h3>
              <p className="text-[9px] md:text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-4 md:mb-6">Atleta {selectedUser?.level} • {selectedUser?.points} Pontos</p>

              <div className="grid grid-cols-2 gap-2 md:gap-3 w-full">
                <button
                  onClick={() => openNotify(isUserBirthday ? 'BIRTHDAY' : isUserInactive ? 'CHURN' : 'GENERAL')}
                  className="h-12 md:h-14 bg-primary text-slate-900 font-black text-[10px] uppercase tracking-widest rounded-2xl shadow-xl shadow-primary/20 flex items-center justify-center gap-1 md:gap-2 hover:scale-[1.02] active:scale-95 transition-all"
                >
                  <span className="material-symbols-outlined text-base md:text-lg">campaign</span>
                  Notificar
                </button>
                <button
                  onClick={() => setIsAthleteProfileOpen(true)}
                  className="h-12 md:h-14 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 font-black text-[10px] uppercase tracking-widest rounded-2xl dark:text-white hover:bg-slate-200 transition-all flex items-center justify-center gap-1 md:gap-2"
                >
                  <span className="material-symbols-outlined text-base md:text-lg">account_circle</span>
                  Perfil
                </button>
              </div>
            </div>

            <div className="space-y-4 md:space-y-8 flex-1 overflow-y-auto custom-scrollbar pb-4">
              <div>
                <h4 className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 md:mb-6 flex items-center gap-2">
                  <span className="material-symbols-outlined text-base md:text-lg text-primary">monitoring</span>
                  Frequência Semanal
                </h4>
              ...
              <div>
                <h4 className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 md:mb-6 flex items-center gap-2">
                  <span className="material-symbols-outlined text-base md:text-lg text-primary-blue">history_edu</span>
                  Timeline Recente
                </h4>
                <div className="space-y-4 md:space-y-6 pl-4 border-l-2 border-slate-100 dark:border-slate-800 ml-2">
                  {[
                    ...
                    <div key={i} className="relative group">
                      <div className={`absolute -left-[27px] top-1 size-5 rounded-full border-4 border-white dark:border-surface-dark shadow-md flex items-center justify-center ${h.color === 'green' ? 'bg-green-500' : h.color === 'primary' ? 'bg-primary text-slate-900' : 'bg-slate-400'
                        }`}>
